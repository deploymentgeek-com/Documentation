<#
.SYNOPSIS
    Removes a device direct membership rule from a named collection by the device name (within a TS)
.DESCRIPTION
    Removes a device direct membership rule from a named collection by the device name (within a TS)
...
.PARAMETER CollectionName
    Name of the collection to remove device from
.NOTES
    Script name: Remove-ComputerFromCollection.ps1
    Author:      Curtis Ling
...
param(
    ...
    [string]$SMSProvider,
    [parameter(Mandatory=$true, HelpMessage=" Name of the collection to remove device from")]
    [ValidateNotNullorEmpty()]
...
$LogFile        = $LogPath + "\RemoveComputerFromCollection.log"
...
ForEach ($ResourceID in $ResourceIDs) {
    ForEach ($CollectionRule in $Collection.CollectionRules) {
        ...
        If ($CollectionRule.ResourceID -like $ResourceID) {
            Write-LogFile -Value "Removing '$($DeviceName)' from '$($CollectionName)"
