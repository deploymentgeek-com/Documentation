<#
.SYNOPSIS
    Adds a device direct membership rule to a named collection by the device name (within a BareMetal TS)
.DESCRIPTION
    Adds a device direct membership rule to a named collection by the device name (within a BareMetal TS)
...
.NOTES
    Script name: Add-ComputerToCollection-BareMetal.ps1
    Author:      Curtis Ling
...
$DeviceName     = ($TSEnv.Value('OSDComputername'))
